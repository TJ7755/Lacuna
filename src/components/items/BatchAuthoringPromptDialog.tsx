  ...
  async function copyPrompt() {
    ...
    try {
      await navigator.clipboard.writeText(
        buildBatchGenerationPrompt({ notes, topic, level, itemCount, round }),
      );
    ...
    >
      ...
      >
        ...
        <header className="relative flex items-start justify-between border-b border-line px-6 py-5">
          <div>
            <h2 className="font-display text-2xl">Generate item batch</h2>
            <p className="mt-1 text-sm text-ink-soft">Build a prompt for {courseName}.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
