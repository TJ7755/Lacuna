...
const CARDS: PracticeCard[] = [
  {
    ...
    face: (
      <svg viewBox="0 0 48 48" className="size-12 text-accent" fill="none" stroke="currentColor" strokeWidth="1.5">
    ...
    face: (
      <div className="w-full max-w-[14rem] rounded-[8px] border border-line-strong bg-paper px-4 py-3 text-left">
        ...
        <p className="mt-2 text-sm text-ink">
          ...
          <span className="rounded bg-accent-soft px-1.5 py-0.5 font-medium text-accent-ink">[…]</span>.
        </p>
      </div>
    ),
  },
  ...
  },
  {
    ...
    face: (
      <svg viewBox="0 0 120 48" className="h-12 w-28 text-accent" fill="none" stroke="currentColor" strokeWidth="2">
        <path
  ...
  {
    name: 'Portable',
    detail: 'Import and export a whole course as a single JSON file — or bring an Anki deck across.',
    face: (
      <svg viewBox="0 0 48 48" className="size-12 text-accent" fill="none" stroke="currentColor" strokeWidth="1.5">
...
export function PracticeDeck({ onComplete }: { onComplete?: () => void }) {
  ...
  return (
    <div className="shadow-paper mt-2 overflow-hidden rounded-[10px] border border-line-strong bg-surface">
      ...
      <div className="grid min-h-52 place-items-center px-6 py-8 text-center">
        {finished ? (
          ...
          <div key={card.name} className="animate-demo-flip flex w-full max-w-md flex-col items-center">
            <div className="grid min-h-16 place-items-center">{card.face}</div>
            <h3 className="mt-5 font-body text-lg font-semibold tracking-normal text-ink">{card.name}</h3>
            <p className="mt-2 text-sm leading-relaxed text-ink-soft">{card.detail}</p>
            <button
              type="button"
              onClick={next}
              className="shadow-paper shadow-paper-hover mt-6 inline-flex min-h-11 items-center rounded-[10px] bg-accent px-8 text-sm font-medium text-accent-fg"
            >
      ...
      {!finished && remaining > 1 && (
        <div className="pointer-events-none relative -mt-2 mb-4 flex justify-center gap-0" aria-hidden>
