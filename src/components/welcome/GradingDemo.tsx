...
const CARDS: DemoCard[] = [
  ...
  { kind: 'cloze', front: 'FSRS schedules reviews so memory peaks on {{c1::exam day}}.', back: 'exam day' },
  { kind: 'basic', front: 'retrievability — noun', back: 'The probability you can recall a card right now.' },
];
...
export function GradingDemo({ onComplete }: { onComplete?: () => void }) {
    ...
    <div className="shadow-paper mt-6 overflow-hidden rounded-[10px] border border-line-strong bg-surface">
      <div className="flex items-center justify-between border-b border-line px-5 py-2.5">
        <span className="font-mono text-[11px] uppercase tracking-[0.18em] text-ink-faint">
          {finished
            ? 'Session complete'
            : `Try it — card ${cardIndex + 1} of ${total}`}
        </span>
