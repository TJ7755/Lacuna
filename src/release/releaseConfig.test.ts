...
describe('release configuration', () => {
  ...
  it('retains required first-party action sources throughout CI and release workflows', () => {
    ...
    expect(namedAction(releaseWorkflow, 'build-win', 'Upload Windows artefacts')).toBe(
      'actions/upload-artifact',
    );
    expect(namedAction(releaseWorkflow, 'publish-draft', 'Download lacuna-win-x64')).toBe(
      'actions/download-artifact',
    );
  });
