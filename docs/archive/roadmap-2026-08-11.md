# Lacuna — Post-Course-Architecture Plan (historical archive)

> Archived 11 August 2026. This is the former all-in-one roadmap and implementation
> diary. It is retained for historical design rationale and old section references;
> it is not the current work queue. See `docs/next_plan.md` for the current roadmap.
...
12. **Arc 11 — Item-type generalisation and authored mark schemes** (delivered;
    model-dependent authoring experiments and deliberate post-arc deferrals recorded)
...
> **Status: delivered (formally closed 11 August 2026).** Numeric and working payloads author, verify,
...
> export/import. Two later free-tier trials exposed and verified fixes to the clipboard
> boundary. Model quality and free-tier latency remain external product research, not an
> acceptance gate for the delivered offline authoring system.
...
## 11.9 Resolved slice-1 decisions and deferred follow-ups
...
6. **A tuple answer kind is deferred beyond Arc 11.**
   ...
   Any future proposal must decide whether v1's one-value-per-answer model should acquire a
   first-class ordered or named tuple — a coordinate pair, a solution set, a quantity with a unit — so that
...
3. The clipboard authoring boundary has been exercised with deterministic sample output and two
   free-tier model trials. The first trial on 29 July 2026 did not meet the proposed ten-minute
   convenience benchmark. Its response was rejected outright because the model closed the
   ...
   returned eight items of which six initially validated. Both fixes held: the model mirrored the
   closing delimiter again and the block parsed anyway, and no answer came back as a tuple. The two
   ...
   The ten-minute free-tier benchmark is retired as an Arc 11 gate. It depends on an unspecified
   third-party model, service latency and output quality that Lacuna neither ships nor controls.
   The trials served their useful purpose by finding defects at Lacuna's parse and verification
   boundaries. Further model-convenience work requires fresh user evidence and is not unfinished
   Arc 11 implementation.
...
**Closure decision (11 August 2026):** criteria 1, 2 and 4 describe the shipped product and are
met. Criterion 3's Lacuna-controlled boundary is also delivered; its external ten-minute target
was an invalid release gate and has been retired, not waived. Tuple answers, scaffold items,
cursor-aware fraction entry, structure-aware equation editing, optimal scheme-line matching and
generated item families are separate parked proposals. None keeps this arc open.
...
> [docs/UX-MAP.html](../UX-MAP.html), including the flows that Arc 10's original
...
Treat [docs/UX-MAP.html](../UX-MAP.html) as the audit baseline for this arc. Every
...
1. Re-run the highest-value scenarios from docs/UX-MAP.html against the current
...
6. Update docs/UX-MAP.html and docs/UX-MAP.json, Help, README/SPEC where relevant, and the website
   checklist.
...
7. The scenarios and regression checks in docs/UX-MAP.html pass on desktop and
