...
A local-first revision application built around the **FSRS-6** spaced-repetition
...
with `--mcp-companion` and the profile arguments generated by Lacuna. Preserve the complete
configuration, especially when using an isolated or custom profile. For a default-profile
installation with Claude Code on macOS:
...
[The documentation index](docs/README.md) maps current references and historical evidence.
